    ...
    * demos archive : crawl the local governments web sites and archive the web documents that matches with the classification
    ...
    def launch(self):
        if self.command_line_arguments.__len__() == 0 or self.command_line_arguments[0] == '-h':
            ...
            self.start_training_launcher(self.command_line_arguments[2:])
        elif self.command_line_arguments[0] == 'archive':
            self.start_archiving_launcher(self.command_line_arguments[2:])
        else:
    ...
    def print_root_manual_page():
        ...
        print('            see training help page to see the options in details : "demos train -h"')
        print('')
        print('  "archive" : a set of tools to archive documents classified as official reports')
        print('              see archiving help page to see the options in details : "demos archive -h"')
        print('')

    def start_training_launcher(self, options):
        pass

    def start_archiving_launcher(self, options):
        pass


if __name__ == '__main__':
    launcher = GlobalLauncher(sys.argv[1:])
    launcher.launch()
